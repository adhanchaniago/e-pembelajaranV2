...
class Ujian extends CI_Controller
{
	...
	public function save2($mapel_id, $topik_id, $jenis_soal)
	{
		$m 		= $this->input->post('mapel_id', true); //1
		$t	 	= $this->input->post('topik', true);
		if ($this->input->post('jenis_soal') == 'pilgan') { //2
			$this->validasi($m, $t); //3
		} else {
		...
		$jumlah_soal 	= $this->input->post('jumlah_soal', true);
		$tgl_mulai 		= $this->convert_tgl($this->input->post('tgl_mulai', 	true));
		$tgl_selesai	= $this->convert_tgl($this->input->post('tgl_selesai', true)); //6
		$waktu			= $this->input->post('waktu', true);
		$jenis_soal		= $this->input->post('jenis_soal', true);
		$jenis			= $this->input->post('jenis', true);
		$id_soal		= $this->input->post('soal', true);
		...
		if ($this->form_validation->run() === FALSE) { //7
			$data['status'] = false;
			$data['errors'] = [
				'nama_ujian' 	=> form_error('nama_ujian'),
				'topik'		 	=> form_error('topik'),
...
//1 = 1,2,3,5,6,7,12 ======soal pilgan terjadi error validasi ===== menampilkan data json (status: false dan error validation)
//2 = 1,2,4,5,6,7,12 ======soal essay terjadi error validasi ===== menampilkan data json (status: false dan error validation) 
//3 = 1,2,4,5,6,8,9,11,12 ======= soal pilgan berhasil ===== menampilkan data json (status: true, guru_id, mapel_id, dan, token)
//4 = 1,2,4,5,6,8,10,11,12 ======= soal essay berhasil  ===== menampilkan data json (status: true, guru_id, mapel_id, dan, token)
